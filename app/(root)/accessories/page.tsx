    ...
    <main className="mx-auto max-w-[1280px] px-6 py-20 md:px-12 md:py-28">
      ...
      <div className="mb-14 flex flex-col justify-between gap-8 md:flex-row md:items-end">
        <h1 className="max-w-2xl font-display text-6xl font-medium leading-[.9] tracking-[-.07em] sm:text-8xl">
          The details
          <br />
          <em className="font-serif font-normal text-[#d8f36a]">matter.</em>
        </h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {accessoryCategories.map((category) => (
          <Link
            className="group relative flex min-h-56 flex-col justify-end overflow-hidden rounded-2xl border border-white/10 bg-white/[0.06] p-7 backdrop-blur-md transition hover:-translate-y-1 hover:border-[#d8f36a]/50"
            href={`/accessories/${category.slug}`}
            key={category.slug}
          >
            <span className="absolute right-[-10px] top-3 font-display text-[170px] font-bold leading-none text-[#d8f36a]/10 transition group-hover:text-[#d8f36a]/20">
              {category.letter}
            </span>
            <span className="relative mb-2 font-display text-2xl">
              {category.name}
            </span>
            <span className="relative max-w-[220px] text-xs leading-5 text-white/40">
              {category.detail}
            </span>
            <span className="absolute right-7 top-7 text-xl text-[#d8f36a]">
              ↗
            </span>
          </Link>
        ))}
